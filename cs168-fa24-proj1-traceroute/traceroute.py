...
def traceroute(sendsock: util.Socket, recvsock: util.Socket, ip: str) \
    ...
    for ttl in range(1, TRACEROUTE_MAX_TTL+1):
        sendsock.set_ttl(ttl) # set TTL for all susbsequent packets
        sendsock.sendto("Testing_1_2_3".encode() , (ip, TRACEROUTE_PORT_NUMBER))
        print("Waiting for response")
        if recvsock.recv_select() == True: # True if we receive a packet response
            print("We got a response")
            # buf = the raw bytes of the packet 
            #  address = (ip, port): a tuple containing the IP address and port that sent the packet.
            buf, address = recvsock.recvfrom() # receives single incoming packet: 
            print(f"Packet bytes in hex: {buf.hex()}")
            print(f"Packet address: {address[0]}")
            print(f"Packet port: {address[1]}")
